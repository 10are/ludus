...
class _MainStoryScreenState extends State<MainStoryScreen> {
  ...
  List<Map<String, dynamic>> _getDialogues() {
    ...
    if (widget.event['path_specific'] != null) {
      final pathSpecific =
          widget.event['path_specific'] as Map<String, dynamic>;
      String pathKey = 'none';
  ...
  void _selectChoice(Map<String, dynamic> choice) {
    ...
    if (choice['requires'] != null) {
      ...
      if (requires['min_gladiators'] != null) {
        if (game.state.gladiators.length <
            (requires['min_gladiators'] as int)) {
      ...
      if (requires['min_family_loyalty'] != null) {
        if (game.state.mainStory.familyLoyalty <
            (requires['min_family_loyalty'] as int)) {
    ...
    if (choice['requires_path'] != null) {
      ...
      if (requiredPath == 'vengeance' &&
          game.state.mainStory.path != StoryPath.vengeance) {
        return;
      }
      if (requiredPath == 'loyalty' &&
          game.state.mainStory.path != StoryPath.loyalty) {
        return;
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    if (_showResult) {
      ...
      return RomanDialogueScreen(
        ...
        topRightWidget: RomanWeekBadge(
          week: game.state.week,
          customText: _chapterText,
        ),
        choices: [DialogueChoice(text: 'Devam', onSelect: widget.onComplete)],
      );
    }
    ...
    if (_showChoices) {
      ...
      final List<DialogueChoice> builtChoices;
      if (choices.isEmpty) {
        builtChoices = [
          DialogueChoice(
            text: 'Devam',
        ...
        builtChoices =
            choices.map((choice) {
              ...
              if (requiresPath != null) {
                if (requiresPath == 'vengeance' &&
                    game.state.mainStory.path != StoryPath.vengeance) {
                  isAvailable = false;
                }
                if (requiresPath == 'loyalty' &&
                    game.state.mainStory.path != StoryPath.loyalty) {
                  isAvailable = false;
                }
              }
              if (choice['requires'] != null) {
                final requires = choice['requires'] as Map<String, dynamic>;
                if (requires['min_gold'] != null &&
                    game.state.gold < (requires['min_gold'] as int)) {
                  isAvailable = false;
                }
              }
